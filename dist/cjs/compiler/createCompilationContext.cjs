...
function normalizeSpace(space) {
  return responsiveValueMap.responsiveValueMap(space, val => {
...
function prepareDevices(configDevices) {
  const devices$1 = []; // let's make devices copy
  devices.DEFAULT_DEVICES.forEach(defaultDevice => {
    devices$1.push({
      ...defaultDevice
    });
  });
    ...
    devices$1.forEach((device, index) => {
      ...
      if (configDevice) {
        ...
        if (configDevice.startsFrom && index > 0) {
          const previousDevice = devices$1[index - 1];
...
function createCompilationContext(config, contextParams, rootComponentId) {
  ...
  const components = [$text.textEditableComponent, $richText.richTextEditableComponent, $richTextBlockElement.richTextBlockElementEditableComponent, $richTextLineElement.richTextLineElementEditableComponent, $richTextPart.richTextPartEditableComponent, {
...
//# sourceMappingURL=createCompilationContext.cjs.map
