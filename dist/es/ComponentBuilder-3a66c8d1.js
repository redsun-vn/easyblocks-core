...
import React, { useState, useRef, useEffect, startTransition, useMemo, useContext, createContext, Fragment } from 'react';
import { z } from 'zod';
...
/**
 * Renders a list of children progressively:
 *
 * 1. First render: only `initialCount` children are mounted (fast first paint).
 * 2. After paint, remaining children are added in batches of `batchSize` using
 *    `startTransition` so React can yield to the browser between batches
 *    (user input, animations, scrolling stay responsive).
 * 3. Continues until all children are rendered.
 *
 * This breaks the "render 50 components in one synchronous pass" problem
 * without any changes to the component tree structure.
 */
function ProgressiveList(_ref) {
  let {
    children,
    initialCount = 3,
    batchSize = 2
  ...
  useEffect(() => {
    ...
    // Schedule the next batch after the browser has painted the current one.
  ...
  // On children length change (e.g. recompilation), reset if needed.
  useEffect(() => {
  ...
  if (visibleCount >= total) {
    // All children visible — return the full array directly (no overhead).
    return /*#__PURE__*/React.createElement(React.Fragment, null, children);
  }
  return /*#__PURE__*/React.createElement(React.Fragment, null, children.slice(0, visibleCount));
}
...
function getCompiledSubcomponents(id, compiledArray, contextProps, schemaProp, path, meta, isEditing, components) {
  ...
  if (!isEditing && elements.length > 3) {
    return /*#__PURE__*/React.createElement(ProgressiveList, {
      initialCount: 3,
      batchSize: 2
    }, elements.map((el, i) => /*#__PURE__*/React.isValidElement(el) ? el : /*#__PURE__*/React.createElement(Fragment, {
      key: i
    }, el)));
  }
  return elements;
